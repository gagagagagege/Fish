...
1. **Renderer2D 的设计** —— 看，理解，然后自己按 Vulkan 实现
...
- [ ] **Fish 项目提交 + 开分支**
      现在所有东西都在 `main` 上，一堆未跟踪文件（`Fish/src/`、`Playground/`、`Creat.bat`、`diary.txt`），
      没法回滚也没法 review。开 `vulkan-backend` 分支再动手
- [ ] **`D:\vulkan_project` 执行 `git init` + 首次提交**
      这个项目**根本没有 git**，而 M1 重构牵动 6 个文件 + 十几个调用点
- [ ] **开始刷算法**，每天固定时间，不要等
- [ ] **投日常实习**，不用等大厂，小厂 / 创业公司 / 远程都行
